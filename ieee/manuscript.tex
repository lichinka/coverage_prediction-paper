...
\thanks{Lucas~Benedi\v{c}i\v{c} is with the Research and Development department, Telekom Slovenije, d.d., Cigaletova 15, SI-1000, Ljubljana, Slovenia, e-mail: \protect\href{mailto:lucas.benedicic@telekom.si}{lucas.benedicic@telekom.si}, www: \protect\href{http://csd.ijs.si/benedicic/}{http://csd.ijs.si/benedicic/}%
}%
\thanks{Felipe~A.~Cruz and Tsuyoshi~Hamada are with the Nagasaki Advanced Computer Center, Nagasaki University, 1-14 Bunkyo-machi, Nagasaki-city, Nagasaki, 852-8521, Japan, e-mail: \protect\href{mailto:fcruz@nacc.nagasaki-u.ac.jp}{fcruz@nacc.nagasaki-u.ac.jp}, \protect\href{mailto:hamada@nacc.nagasaki-u.ac.jp}{hamada@nacc.nagasaki-u.ac.jp}.%
}%
\thanks{Peter~Koro\v{s}ec is with the Computer Systems Department, Jo\v{z}ef Stefan Institute, Jamova cesta 39, SI-1000, Ljubljana, Jamova cesta 39, SI-1000, Ljubljana, e-mail: \protect\href{mailto:peter.korosec@ijs.si}{peter.korosec@ijs.si}.%
...
installed at fixed locations, for this reason one of the primary objectives
of mobile-network planning is to efficiently use the allocated frequency
band to assure that the whole of the geographic area of interest can
be satisfactorily reached with the radio stations of the network.
To this end, radio-coverage prediction tools are of great importance
as it allows the network engineers to test different network configurations
before physically implementing the changes. Nevertheless, radio-coverage
prediction is a complex task due to the wide range of various combinations
of hardware and configuration parameters which have to be analyzed
in the context of different environments. The complexity of the problem
means that radio-coverage prediction can be a computationally-intensive
and time-consuming task, hence the importance of fast and accurate
prediction tools.
...
its work-flow to it, when the opposite situation should be preferable.
...
an ideal candidate for calculating radio-predictions of real mobile
networks containing thousands of transmitters. And also for the scientific
research community, since our design may be used as a template for
parallelization of computationally-expensive tasks within the GRASS
environment.
...
steps of: parallel decomposition of the radio-coverage prediction
problem for real networks and the distribution of the computational
load among the computing nodes that belong to the cluster.
...
as an extension of the well-known COST Hata model \cite{Sarkar_Survey_of_radio_propagation_models:2003,Shabbir_Comparison_of_radio_propagation_models:2011},
...
as in Equation (\ref{eq:cost231_NLOS}).
...
the terrain profile is used for LOS determination. Besides, the wave-guide
...
a Geographical Information System (GIS). This GIS software was originally
...
All input data are read in the first step (``Read input data'' in
\prettyref{fig:serial_version_flow_diagram}), e.g. digital elevation
...
within the specified transmission radius (``transmission radius''
...
(``Calculate LOS/NLOS'' in \prettyref{fig:serial_version_flow_diagram}).
...
or Equation (\ref{eq:cost231_NLOS-1}) is respectively applied (``Apply
COST-231, LOS'' or ``Apply COST-231, NLOS'' in \prettyref{fig:serial_version_flow_diagram}).
...
and its influence over the isotropic path-loss calculation (``Calculate
...
in its own database table (``Save transmitter path-loss to DB''
in \prettyref{fig:serial_version_flow_diagram}), thus considerably
enhancing the write performance during result dumping. This is accomplished
by connecting the standard output of the developed module with the
standard input of a database client. Naturally, the generated plain
text should be understood by the database server itself.
...
created in this step (``Create final coverage prediction'' in \prettyref{fig:serial_version_flow_diagram}).
...
prediction of several transmitters. The color scale is given in }}\texttt{\textit{\emph{\footnotesize dBm}}}\textit{\emph{,
...
and manipulating light-weight processes or threads. By using POSIX
threads, multiple threads can exist within the same process while
sharing its resources. For instance, an application using POSIX threads
can: execute multiple threads in parallel by using the cores of multi-core
processor, or use the system resources more effectively by avoiding
process execution-halt due to I/O latency by using one thread for
computing while a second thread waits for an I/O operation to complete. 
...
or MPI \cite{Gropp_Using_MPI:1999}. MPI is a message-passing standard
...
the available computing nodes (see the ``Dynamic worker-process spawning''
step in \prettyref{fig:master_process}), based on the amount of transmitters
...
than the number of transmitters to be processed. However, most often
there are more transmitters than available computing nodes, therefore,
the master process can assign several transmitters to each of the
worker processes. For distributing the work among the worker processes,
the master process proceeds to decompose the loaded raster data into
arrays of basic-data-type elements, e.g. floats or doubles, before
dispatching them to the multiple worker processes (see the ``Input
data broadcasting'' step in \prettyref{fig:master_process}). The
decomposition of the data applies to the digital-elevation and the
clutter data only. In the next step, the master process starts a message-driven
processing loop (see the ``Processing loop'' step in \prettyref{fig:master_process}),
which main task is to assign and distribute the configuration data
of the different transmitters among idle worker processes.
...
the processing loop itself (see the ``Any worker still on?'' step
in \prettyref{fig:processing_loop_in_master_process}). The active
worker processes inform the master process they are ready to compute
by sending an idle message (see the ``Wait for idle worker'' step
...
configuration of the transmitter to be processed (see the ``Send
keep-alive message'' and ``Send transmitter data'' steps, respectively,
in \prettyref{fig:processing_loop_in_master_process}). This is only
done in case there are transmitters for which the coverage prediction
has yet to be calculated (see the ``Any transmitters left?'' step
in \prettyref{fig:processing_loop_in_master_process}). The processing
loop of the master process continues to distribute transmitter data
among worker processes, which asynchronously become idle as they finish
the coverage-prediction calculations for the transmitters they have
been assigned by the master process. When there are no more transmitters
left, all the worker processes announcing they are idle will receive
a shutdown message from the master process, indicating them to stop
running (see the ``Send stop message'' step in \prettyref{fig:processing_loop_in_master_process}).
The master process will keep doing this until all worker processes
have finished (see the ``Any worker still on?'' step in \prettyref{fig:processing_loop_in_master_process}),
...
the final output of the calculation, e.g. a raster map (see the ``Create
final coverage prediction'' step in \prettyref{fig:master_process}).
...
time (see the ``Receive broadcasted data'' step in \prettyref{fig:worker_process_flow_diagram}).
...
(see the ``Send idle message'' step in \prettyref{fig:worker_process_flow_diagram}).
If the master process does not instruct to stop processing (see the
``Has stop message arrived?'' step in \prettyref{fig:worker_process_flow_diagram}),
the worker process collects the transmitter configuration sent (see
the ``Receive transmitter data'' step in \prettyref{fig:worker_process_flow_diagram}).
However, in case a stop message is received, the worker process will
wait for result-dumping threads to finish (see the ``Wait for result-dump
threads'' step in \prettyref{fig:worker_process_flow_diagram}) before
shutting down. The coverage calculation itself follows a similar design
as the serial version (see the ``Coverage calculation'' step in
\prettyref{fig:worker_process_flow_diagram}) and it is executed for
...
to a database table (see the ``Threaded save path-loss to DB'' step
in \prettyref{fig:worker_process_flow_diagram}). It is important
to note that there is no possibility of data inconsistency due to
the saving task being executed inside a thread, since path-loss data
from different workers belong to different transmitters and are mutually
exclusive. For this reason, there is no need of any concurrent I/O
overlapping \cite{Liao_Scalable_design_and_implementations_for_MPI_parallel_overlapping:2006}.
...
are simplicity and neglectable overhead, which contrast with more
elaborated approaches for parallel-task allocation in heterogenous
clusters \cite{Bosque_A_parallel_computational_model_for_heterogenous_clusters:2006}.
...
there are many different ways a parallel program can execute, because
the steps from the different processes can be interleaved in various
ways and a process can make non-deterministic choices \cite{Siegel_Verification_of_halting_properties_for_MPI_programs:2007},
...
\texttt{gcc} compiler, version 4.4.4.
...
configuration parameters of these 10 transmitters are taken from the
UMTS network deployed in Slovenia by Telekom Slovenije, d.d. The path-loss
predictions are calculated using the COST-231. The digital elevation
model has an area of 20,270~km$^{2}$, with a resolution of 25~m$^{2}$,
the same as the clutter data, which contains different levels of signal
loss based on the land usage. For all the points within a transmission
radius of 20~km around each transmitter, we assume that the receiver
is positioned 1.5~m above the ground, and the frequency is set to
2040~MHz.
...
per worker/core: $\{5,10,20,40,80\}$, and we increase the number
of workers/cores from 1 to 128 in powers of 2.
...
increasing both the problem size and the number of cores, results
in a roughly constant time to solution. Therefore, with this set of
experiments, we would like to investigate how well-suited the coverage-prediction
...
measurements express wall-clock times in seconds. Wall-clock time
represents real time that elapses from the start of the master process
to its end, including time that passes waiting for resources to become
available. They are plotted in \prettyref{fig:weak_scalability_time},
...
of factors, namely:
\begin{itemize}
\item the overhead time of the serial sections of the master process grow
proportionally with the number of cores, although the total contribution
of this overhead remains low for large problem sizes;
\item communication overhead grows linearly with the number of cores used.
\end{itemize}
...
we increase the number of workers/cores from 1 to 128 in powers of
...
per core approaches one. It can be observed in \prettyref{tab:results_strong_scaling}
...
\noindent where $NTX$ is the number of transmitters processed, i.e.
the problem size, and $NP$ is the number of cores executing the worker
...
plots for four problem instances, namely 256, 1024, and 4096 transmitters,
...
of time as we increase the number of cores, since the total wall-clock
...
an idle message (see the ``Send idle message'' step in \prettyref{fig:master_worker_communication}),
...
the overhead of the message passing at the same time the coverage
...
as for strong scalability of Section \ref{sub:Strong-scalability},
...
95\% of a perfect load-balancing factor, and within 10\% for problem
...
the parallel process, thus improving the load-balancing factors.
...
to support, for example, other propagation models and post-processing
algorithms. This characteristic defines a clear advantage when compared
to commercial and closed-source tools.
...
\begin{IEEEbiographynophoto}{Tsuyoshi Hamada} received his Ph.D.
in 2006 from the University of Tokyo. From 2006 to 2008 he was a Special
Postdoctoral Researcher at RIKEN. In 2008 he became an Assistant Professor
at Nagasaki University, and was promoted to Associate Professor in
2010. With his work on massively parallel architectures he was awarded
the Gordon Bell prize for prize/performance in 2009 and a Gordon Bell
prize Honorable mention in 2010. He is currently the deputy director
of the Nagasaki Advanced Computing Center at Nagasaki University,
Japan.
...
interest include combinatorial and numerical optimization with modern
